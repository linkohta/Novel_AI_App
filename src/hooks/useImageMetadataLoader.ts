...
export function useImageMetadataLoader({
  ...
  setQueueItems,
  setStatus,
}: UseImageMetadataLoaderParams) {
  async function handleLoadImageMetadata(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (meta.width) setWidth(String(meta.width));
      if (meta.height) setHeight(String(meta.height));
  ...
  async function handleLoadQueueItemImageMetadata(index: number, e: ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    e.target.value = ''; // 次回も同じファイルを選択できるようにする
    if (!file) return;
    try {
