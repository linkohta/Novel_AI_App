...
async function parsePngChunks(arrayBuffer: ArrayBuffer): Promise<ParsedPng> {
...
export interface ExtractedCharacterPrompt {
  prompt: string;
  negativePrompt: string;
}

export interface ExtractedNovelAiMetadata {
  prompt: string;
  negativePrompt: string;
  steps: string;
  scale: string;
...
export async function extractNovelAiMetadata(file: File): Promise<ExtractedNovelAiMetadata | null> {
  const arrayBuffer = await file.arrayBuffer();
  const { width, height, texts } = await parsePngChunks(arrayBuffer);

  let comment: CommentJson = null;
  if (texts.Comment) {
    try {
  ...
  const result: ExtractedNovelAiMetadata = {
  ...
  if (comment?.v4_prompt?.caption?.char_captions) {
    ...
    result.characters = positives.map((c: CommentJson, i: number) => ({
      prompt: c.char_caption || '',
      negativePrompt: negatives[i]?.char_caption || '',
    }));
  } else if (Array.isArray(comment?.characterPrompts)) {
    result.characters = comment.characterPrompts.map((c: CommentJson) => ({
      prompt: c.prompt || '',
      negativePrompt: c.uc || '',
    }));
  }
