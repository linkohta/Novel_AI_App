...
if (!window.api) {
  ...
  function saveJson(key: string, data: JsonValue) {
    return Preferences.set({ key, value: JSON.stringify(data) });
  }

  function makeNamedListApi(key: string) {
    return {
      load: () => loadJson<NamedListItem[]>(key, []),
      save: async (item: { name: string; text: string }) => {
        const list = await loadJson<NamedListItem[]>(key, []);
        list.push({ id: uuid(), name: item.name, text: item.text });
        await saveJson(key, list);
        return list;
      },
      update: async (item: NamedListItem) => {
        const list = await loadJson<NamedListItem[]>(key, []);
      ...
      remove: async (id: string) => {
        const list = (await loadJson<NamedListItem[]>(key, [])).filter((i) => i.id !== id);
        await saveJson(key, list);
        return list;
      },
    };
  ...
  function makeGenericListApi(key: string) {
    return {
      load: () => loadJson<GenericListItem[]>(key, []),
      save: async (item: JsonValue) => {
        const list = await loadJson<GenericListItem[]>(key, []);
        list.push({ id: uuid(), ...item });
        await saveJson(key, list);
        return list;
      },
      update: async (item: GenericListItem) => {
        const list = await loadJson<GenericListItem[]>(key, []);
        const target = list.find((i) => i.id === item.id);
        if (target) {
      ...
      remove: async (id: string) => {
        const list = (await loadJson<GenericListItem[]>(key, [])).filter((i) => i.id !== id);
        await saveJson(key, list);
        return list;
      },
    };
  ...
  async function savePromptInfo(params: GenerateImageParams): Promise<boolean> {
    const body = buildRequestBody(params);
  ...
  async function getSubscriptionInfo(apiKey: string): Promise<SubscriptionInfo> {
  ...
  const favoritesApis: Record<FavoriteKind, ReturnType<typeof makeGenericListApi>> = {
  ...
  const api: WindowApi = {
    ...
    saveSettings: (settings) => saveJson(SETTINGS_KEY, settings).then(() => true),
    generateImage,
    savePromptInfo,
    getSubscriptionInfo,
    openOutputFolder,
