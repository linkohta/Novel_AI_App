...
- **AIポーション（Vibe Transfer）**: NovelAI公式サイトの「AIポーション」に相当する参照画像による画風・雰囲気の転写機能。「AIポーション（Vibe Transfer）」セクション（`src/components/VibeTransferSection.tsx`、`src/hooks/useVibeTransfer.ts`）で画像ファイルを選択して参照画像を追加でき、追加した画像ごとにサムネイル・Information Extracted（参照画像からどれだけ情報を抽出するか、0〜1、既定1）・Reference Strength（抽出した情報をどれだけ強く反映するか、0〜1、既定0.6）のスライダーと削除ボタンを持つカードとして一覧表示される。NovelAI APIへは、参照画像が1枚以上ある場合のみ `parameters.reference_image_multiple`（base64文字列の配列、data URLのprefixなし）・`parameters.reference_information_extracted_multiple`（数値の配列）・`parameters.reference_strength_multiple`（数値の配列）という3つの同じ長さ・同じ順序の並行配列として送信し（`shared/novelai.mts` の `buildRequestBody`）、参照画像が0枚の場合はこれら3フィールドを一切送信しない。この形式はV3系・V4系・V5系のどのモデルでも共通であり、モデル種別による分岐は無い。単一プロンプト生成（左パネルの「AIポーション（Vibe Transfer）」セクション）と複数プロンプト連続生成（各行に「＋ 参照画像を追加」欄。`.char-fold` による折りたたみでキャラクタープロンプトと同様に行の縦幅を抑えている）の両方から利用でき、複数プロンプト連続生成では行ごとに独立した参照画像の集合を持つ（`QueueItem.vibeTransferImages`）。単一プロンプト側の参照画像一覧は他の入力欄と同様に `settings.vibeTransferImages` として永続化され、行ごとの参照画像は既存の `queueItems` の永続化に含まれる。複数プロンプトテンプレート機能はプロンプト文字列のみを対象とする設計のため、AIポーションの参照画像はテンプレートの保存・適用の対象に含めていない（適用時は空配列から開始する）。
- **設定の永続化**: 画面に表示されている入力・選択状態は原則としてすべて `settings.json` に保存し、次回起動時に同じ状態へ復元する（`load-settings` / `save-settings` IPC）。対象はAPIキー・プロンプト／ネガティブプロンプト・モデルや画像サイズ等のパラメータ・Quality Tags自動追加のON/OFF・保存先フォルダ・キャラクタープロンプト・AIポーション（Vibe Transfer）の参照画像一覧（`vibeTransferImages`）・複数プロンプト連続生成のリスト（`queueItems`、行ごとのプロンプト／ネガティブプロンプト／枚数／キャラクタープロンプト／AIポーション参照画像を含む）・連続生成／複数プロンプト連続生成の回数や生成間隔（秒）・各機能セクションの折りたたみ状態（`sectionState`）など、UI上のフォーム入力全般に及ぶ。新しく永続化すべき入力欄（フォーム値・回数・間隔など）を画面に追加した場合は、`App.tsx` の設定読み込み（`useEffect` 内の `loadSettings` 呼び出し）と `currentSettings`（保存対象オブジェクトとその依存配列）の両方に必ず追加すること。片方だけ追加すると、保存はされるが起動時に復元されない（またはその逆）状態になる。例外的に永続化しないのはシード値（`seed`、既定で毎回0/ランダムに戻す、旧アプリの挙動を踏襲）のように意図的に「起動のたびにリセットする」設計にしているものだけであり、その場合はコード上にその旨のコメントを残す。
