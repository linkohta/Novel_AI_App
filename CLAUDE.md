...
- **コードコメントも日本語で統一する。** `main.js`/`preload.js`/`shared/`（CommonJS/ESM問わず）・`src/**/*.{js,jsx}` を含むすべてのソースファイルで、英語のコメントを新規に追加しない。既存の英語コメントを見つけた場合も、そのファイルを編集する際に日本語へ書き直す。
